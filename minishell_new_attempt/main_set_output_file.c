...
/*   Updated: 2024/08/02 13:41:00 by ehedeman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_temp_output_as_stdout(t_mini *mini, int reset)
{
	...
	mini->temp_output = open(".output_the_first_cuz_what_the_fuck", O_RDWR | O_CREAT | O_TRUNC, mode);
...
{
	mini->temp_output = open(".output_the_first_cuz_what_the_fuck", O_RDWR);
	if (mini->temp_output < 0)
	{
		printf("minishell: system error.");
		return (-1);
